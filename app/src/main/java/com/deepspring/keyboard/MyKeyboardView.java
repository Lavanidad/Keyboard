...
/**
 * Created by fzy on 2019/6/27.
 */
public class MyKeyboardView extends KeyboardView {

    private Context mContext;
    private Drawable delDrawable;
    private static final int ICON2KEY = 2;//按键的宽高至少是图标宽高的倍数
    ...
    public void onDraw(Canvas canvas) {
        ...
        for(Keyboard.Key key : keys) {
            ...
            if (key.codes[0] == -5 || key.codes[0] == -2 || key.codes[0] == 100860
                    || key.codes[0] == -1 || key.codes[0] == -200 || key.codes[0] == -500
                    || key.codes[0] == -800 || key.codes[0] == -1000)
    ...
    private void drawSpecialKey(Canvas canvas, Keyboard.Key key) {
        ...
        } else if (key.codes[0] == -800 || key.codes[0] == -200 ||
                key.codes[0] == -500 || key.codes[0] == -1000) {
            drawKeyBackground(R.drawable.keyboard_change, canvas, key);
